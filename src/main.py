...
query = f'(diagram generation OR text-to-diagram OR figure generation OR text-to-figure OR code generation OR text-to-code OR (evaluation AND (diagram OR figure OR code OR image))) AND cat:(cs.CL OR cs.CV OR cs.PL) AND submittedDate:[{dt_start} TO {dt_end}]'
